...
func main() {
	// DB Setup
	db, err = sql.Open("mysql", "root:12345@(127.0.0.1:3306)/mydb")
	if err != nil {
		panic(err.Error())
	}
	defer db.Close()
...
func updateBook(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	statement, err := db.Prepare("UPDATE books SET isbn = ?, title = ?, author = ?  WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
	...
	dataMap := make(map[string]string)
	json.Unmarshal(body, &dataMap)
	newIsbn := dataMap["isbn"]
	newTitle := dataMap["title"]
	newAuthor := dataMap["author"]

	_, err = statement.Exec(newIsbn,newTitle,newAuthor, params["id"])
	if err != nil {
		panic(err.Error())
	}
